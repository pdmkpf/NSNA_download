...
header('Content-Disposition: attachment;filename="refernce.xls"');
...
$url = "http://0.0.0.0:9999/NSNA_ExpertFrontEnd/JSP/DownloadExcel.jsp?keyword=".$keyword."&begin=".$begin."&end=".$end."&providers=".$providers."&unit=".$unit;

$xml = simplexml_load_file($url);
//$xml=simplexml_load_file("example.xml");

foreach($xml -> UNIT -> children() as $unit_child) {
	$active_sheet = $objPHPExcel->setActiveSheetIndex(0)->setTitle('Reference');
	$row_num = 2;
	$active_sheet->setCellValue('A1', 'INFOSRC_NAME')
				->setCellValue('B1', 'STN_CONTENT')
				->setCellValue('C1', 'ART_ID');

	foreach($unit_child -> REFERENCE -> children() as $child) {
		$child_name = $child->getName();
		if($child_name == "STN_CTR") {
			//
		} else if ($child_name == "STN") {
			$active_sheet->setCellValueByColumnAndRow(0, $row_num, trim_str($child->INFOSRC_NAME));
			$active_sheet->setCellValueByColumnAndRow(1, $row_num, trim_str($child->STN_CONTENT));
	...
	$active_sheet->setCellValue('A1', 'ART_ID')
				->setCellValue('B1', 'ART_HEADLINE')
				->setCellValue('C1', 'ART_DATE')
				->setCellValue('D1', 'ART_PROVIDER')
				->setCellValue('E1', 'ART_CONTENT');

	foreach($unit_child-> ARTICLE -> children() as $child) {
		$child_name = $child->getName();
		if($child_name == "ART_CTR") {
		} else if ($child_name == "ART") {
